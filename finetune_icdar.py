...
def run_epoch(model, loader, device, max_label_length, criterion_ctc=None, optimizer=None):
    ...
    for images, labels, _, _ in loader:
        ...
        if is_ctc_output(logits, batch_size):
            ...
            loss = criterion_ctc(log_probs, targets_flat, input_lengths, target_lengths)
...
def main():
    parser = argparse.ArgumentParser(description='Fine-tune OCR model on ICDAR Train split.')
    parser.add_argument('--dataset_root', type=str, default='')
    parser.add_argument('--train_split', type=str, default='Train')
    parser.add_argument('--val_split', type=str, default='Test')
    parser.add_argument('--arch', type=str, default='')
    parser.add_argument('--checkpoint', type=str, default='', help='Initial checkpoint (.pth.tar or state_dict)')
    parser.add_argument('--resume', type=str, default='', help='Resume full training state checkpoint')
    ...
    parser.add_argument('--save_path', type=str, default='./save/ICDAR_finetune')
    ...
    train_ds = ICDARWordDataset(dataset_root, args.train_split, transform=transform)
    val_ds = ICDARWordDataset(dataset_root, args.val_split, transform=transform)
    ...
    criterion_ctc = torch.nn.CTCLoss(blank=0, reduction='mean', zero_infinity=True).to(device)

    os.makedirs(args.save_path, exist_ok=True)
    log_path = Path(args.save_path) / 'finetune_icdar.log'
    ...
    with open(log_path, 'a', encoding='utf-8') as log_f:
        log_f.write(f'\n[{datetime.now().isoformat(timespec="seconds")}] Fine-tune ICDAR start\n')
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            train_metrics = run_epoch(
                model, train_loader, device, args.max_label_length,
                criterion_ctc=criterion_ctc, optimizer=optimizer,
            )
            scheduler.step()
            val_metrics = run_epoch(
                model, val_loader, device, args.max_label_length,
                criterion_ctc=criterion_ctc, optimizer=None,
            )

            lr_now = optimizer.param_groups[0]['lr']
            msg = (
                f"Epoch {epoch + 1:03d}/{args.epochs:03d} | "
                f"Arch {args.arch} | LR {lr_now:.3e} | "
        ...
        torch.save(state, Path(args.save_path) / 'last_ckpt.pth.tar')
