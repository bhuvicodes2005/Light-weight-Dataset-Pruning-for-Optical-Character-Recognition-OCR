...
# ── CER singleton (avoids re-instantiating every batch) ───────────────────────
...
def load_model(arch, num_classes):
    ...
    if arch == 'CRNN':
        ...
        return SVTR(img_channel=1, img_height=32, img_width=128, num_class=num_classes)
    elif arch == 'PARSEQ':
...
parser = argparse.ArgumentParser(
    description='Trains OCR models on MJSynth',
...
parser.add_argument('--arch', type=str, default='CRNN',
                    choices=['CRNN', 'ViTSTR', 'SVTR', 'PARSEQ', 'VisionLAN', 'TrOCR'])
parser.add_argument('--max_label_length', type=int, default=25,
                    help='Max sequence length for non-CTC decoders.')
...
parser.add_argument('--la_lr_scale', type=float, default=0.1,
                    help='LR multiplier applied when switching to LA phase.')
...
def is_ctc_model(arch):       return arch in ('CRNN', 'ViTSTR', 'SVTR')
...
def encode_labels(texts, char2idx=CHAR2IDX):
    """Convert a list of strings → (targets_flat, target_lengths) for CTCLoss."""
    encoded, lengths = [], []
    for t in texts:
...
def encode_seq_targets(texts, max_len, char2idx=CHAR2IDX):
    """
    Encode texts as fixed-length sequences with EOS appended, zero-padded.
    Layout per row: [char, char, …, EOS, 0, 0, …]
    index 0 is the padding token (matches ignore_index=0 in the loss).
    """
...
def get_label_lengths(texts, char2idx=CHAR2IDX):
    """Return actual character count (clamped to max_label_length) for each text.
    Used to restrict MLM masking to valid positions only."""
    return torch.tensor(
...
def per_sample_seq_loss(logits, targets, ignore_index=0):
    """Per-sample average cross-entropy for padded sequence targets."""
...
def cer_percent(preds, targets):
    """
    FIX: reuse module-level singleton instead of instantiating CharErrorRate()
    every batch — 2-3x faster.
    """
    try:
        _CER_METRIC.reset()
        return _CER_METRIC(preds, targets).item() * 100.0
...
def load_recorder_from_state(recorder, state):
    ...
    if keys.issubset(state):
        old_losses   = np.array(state['epoch_losses'])    # shape (12, 2)
        old_accuracy = np.array(state['epoch_accuracy'])  # shape (12, 2)
        old_epochs   = old_losses.shape[0]

        if old_epochs < recorder.total_epoch:
            pad = recorder.total_epoch - old_epochs
            recorder.epoch_losses   = np.concatenate(
                [old_losses,   np.zeros((pad, old_losses.shape[1]))],   axis=0)
            recorder.epoch_accuracy = np.concatenate(
                [old_accuracy, np.zeros((pad, old_accuracy.shape[1]))], axis=0)
        else:
            recorder.epoch_losses   = old_losses
            recorder.epoch_accuracy = old_accuracy

        recorder.current_epoch = state['current_epoch']
        # DO NOT restore total_epoch — keep args.epochs value
    return recorder
...
def set_visionlan_phase(model, epoch, lf_epochs, log, optimizer=None):
    """
    Set VisionLAN training phase (LF -> LA).

    FIX: When transitioning to LA phase, reduce LR by `args.la_lr_scale`
    (default 0.1) to prevent catastrophic forgetting of LF-phase weights.
    """
    if not hasattr(model, 'set_lf_phase'):
        return

    if epoch < lf_epochs:
        model.set_lf_phase(True)    # Language-Free
        print_log(f"  [VisionLAN] Phase: Language-Free (epoch {epoch+1}/{lf_epochs})", log)
    else:
        was_lf = model.lf_phase     # True only on the first LA epoch
        model.set_lf_phase(False)   # Language-Aware

        # FIX: One-time LR reduction at the LF→LA boundary
...
def train(train_loader, args, model, criterion, optimizer, scheduler, epoch, log,
    ...
    # FIX: use lists instead of np.concatenate every batch — 5-10x faster
    loss_list  = []
    index_list = []
    y_list     = []
    ...
    for t, batch in enumerate(train_loader):
        ...
        if is_ctc_model(args.arch):
            ...
            # FIX: Pass label_lengths to VisionLAN so MLM only masks valid positions.
        ...
        # ── Dynamics bookkeeping (FIX: append to lists, concat once at end) ──
        loss_list.append(per_sample_loss.detach().cpu().numpy().reshape(-1))
        index_list.append(sample_indices.detach().cpu().numpy().reshape(-1))
    ...
    # FIX: single np.concatenate at end of epoch instead of per-batch
    loss_epoch  = np.concatenate(loss_list)  if loss_list  else None
    index_epoch = np.concatenate(index_list) if index_list else None
    y_epoch     = np.concatenate(y_list)     if y_list     else None
...
def validate(test_loader, args, model, criterion, log, trocr_processor=None):
    ...
    epoch_cer = cer_metric.compute().item() * 100.0
    del cer_metric, pred_texts, target_texts
...
def main():
    ...
    log = open(os.path.join(log_path, f'seed_{args.manualSeed}_log.txt'), 'w')
    ...
    if args.resume:
        ...
        print_log(f"=> resumed from '{args.resume}' (epoch {start_epoch + 1})", log)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        need_h, need_m, need_s = convert_secs2time(
        ...
        save_checkpoint(state, args.save_path, f'epoch_{epoch+1:03d}_ckpt.pth.tar')
        if is_best:
            save_checkpoint(state, args.save_path, 'best_ckpt.pth.tar')
        if epoch + 1 == args.epochs:
            save_checkpoint(state, args.save_path, 'last_ckpt.pth.tar')

        epoch_time.update(time.time() - epoch_start)

        if epoch % 5 == 0 or epoch + 1 == args.epochs:
            recorder.plot_curve(
                os.path.join(args.save_path, f'{args.manualSeed}_curve.png'))
