...
parser = argparse.ArgumentParser(
    description='Train on subset selected via DUAL importance mask',
...
# Mask-based subset selection
parser.add_argument('--mask-path', type=str, required=True,
                    help='Path to dual_mask_T*.npy generated by importance_evaluation.py')
parser.add_argument('--subset-rate', type=float, default=0.3,
                    help='Fraction of data to keep after importance scoring')
parser.add_argument('--keep', type=str, default='highest', choices=['lowest', 'highest'],
                    help='lowest=hard samples, highest=easy/consistent samples')
...
def load_recorder_from_state(recorder, state):
    ...
    if keys.issubset(state):
        old_losses = np.array(state['epoch_losses'])
        old_accuracy = np.array(state['epoch_accuracy'])
...
# ── Mask-based subset selection ───────────────────────────────────────────────
def build_subset_indices(mask_path, subset_rate, keep):
...
def set_visionlan_phase(model, epoch, lf_epochs, log, optimizer=None):
    ...
    if epoch < lf_epochs:
        ...
        if was_lf and optimizer is not None:
            ...
            print_log("  [VisionLAN] Phase: Language-Aware", log)
...
def train(train_loader, args, model, criterion, optimizer, scheduler, epoch, log,
    ...
    for t, batch in enumerate(train_loader):
        ...
        if len(batch) == 3:
            ...
            images, labels = batch
            sample_indices = torch.arange(t * args.batch_size,
        ...
        if is_ctc_model(args.arch):
            ...
            ) / target_lengths.float().clamp_min(1)
            ...
            targets_flat = targets_flat.to(args.device)
            loss = criterion(output.view(-1, NUM_CLASSES), targets_flat)
            per_sample_loss = loss.detach().expand(images.size(0))

        pred_texts = (ctc_decode(output, VOCAB) if is_ctc_model(args.arch)
...
def main():
    ...
    print_log(f'Full train samples  : {total_train}', log)
    print_log(f'Subset train samples: {len(pruned_dataset)} ({len(pruned_dataset)/total_train:.1%})', log)

    args.num_samples = len(pruned_dataset)
