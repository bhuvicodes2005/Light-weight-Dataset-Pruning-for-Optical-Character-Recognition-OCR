...
def main():
    parser = argparse.ArgumentParser(description='Fine-tune OCR model on SVT train split.')
    ...
    parser.add_argument('--save_path', type=str, default='./save/SVT_finetune')
    ...
    train_ds = SVTWordDataset(dataset_root, args.train_split, transform=transform)
    val_ds = SVTWordDataset(dataset_root, args.val_split, transform=transform)
    ...
    log_path = Path(args.save_path) / 'finetune_svt.log'
    ...
    with open(log_path, 'a', encoding='utf-8') as log_f:
        log_f.write(f'\n[{datetime.now().isoformat(timespec="seconds")}] Fine-tune SVT start\n')
        ...
        for epoch in range(start_epoch, args.epochs):
            ...
            # VisionLAN two-stage schedule: LF (mask-free) -> LA (masking enabled)
