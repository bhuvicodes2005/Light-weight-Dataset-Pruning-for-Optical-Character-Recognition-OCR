...
def reduce_output_to_sample_scalar(epoch_output: np.ndarray, epoch_indices: np.ndarray) -> np.ndarray:
    ...
    - 1D: already scalar per sample
    - 2D (N, C): classifier logits → max softmax confidence
    - 3D (N, T, C) or (T, N, C): CTC/seq2seq → geometric mean of per-step max confidence
    """
    output_t    = torch.tensor(epoch_output, dtype=torch.float32)
    num_indices = int(np.asarray(epoch_indices).reshape(-1).shape[0])   # FIX 4 — renamed for clarity
...
def load_trajectories(selected_source, selected_epochs):
    indexes_local = []
    values_local  = []
...
if __name__ == '__main__':
    ...
    try:
        values, indexes = load_trajectories(source, epochs)
    ...
    # FIX 6 — explicitly convert tensor to numpy before saving
